  ...
  const results = useMemo(() => {
    ...
    (db.jobs || []).forEach((j: any) => {
      if (j.client?.toLowerCase().includes(q) || j.phone?.includes(q) || j.address?.toLowerCase().includes(q) || j.desc?.toLowerCase().includes(q) || j.num?.includes(q)) {
        items.push({ type: IS_SOLO_EDITION ? 'Job' : 'עבודה', icon: <Work sx={{ fontSize: 14 }} />, label: `${j.num || '#' + j.id} — ${j.client}`, sub: j.address || j.desc || '', href: '/jobs', color: '#3B82F6' });
      }
    });
    ...
    (db.leads || []).forEach((l: any) => {
      if (l.name?.toLowerCase().includes(q) || l.phone?.includes(q) || l.email?.toLowerCase().includes(q)) {
        items.push({ type: IS_SOLO_EDITION ? 'Lead' : 'ליד', icon: <People sx={{ fontSize: 14 }} />, label: l.name, sub: l.phone || l.email || '', href: '/leads', color: '#F59E0B' });
      }
    });
    ...
    (db.quotes || []).forEach((qt: any) => {
      if (qt.client?.toLowerCase().includes(q) || qt.phone?.includes(q)) {
        items.push({ type: IS_SOLO_EDITION ? 'Quote' : 'הצעה', icon: <Description sx={{ fontSize: 14 }} />, label: `${qt.num || '#' + qt.id} — ${qt.client}`, sub: qt.total ? `₪${qt.total}` : '', href: '/quotes', color: '#8B5CF6' });
      }
    });
    ...
    (db.users || []).forEach((u: any) => {
      if (u.name?.toLowerCase().includes(q) || u.phone?.includes(q) || u.email?.toLowerCase().includes(q)) {
        items.push({ type: IS_SOLO_EDITION ? 'Team' : 'צוות', icon: <Person sx={{ fontSize: 14 }} />, label: u.name, sub: u.role || '', href: '/technicians', color: '#10B981' });
      }
    });
  ...
  return (
    <Box ref={ref} sx={{ position: 'relative', width: open ? { xs: '100%', md: 400 } : 200, transition: 'width 0.3s' }}>
      <TextField
        ...
        placeholder={IS_SOLO_EDITION ? "Search… (Ctrl+K)" : "חיפוש... (Ctrl+K)"}
        size="small"
        fullWidth
        InputProps={{
