      ...
      try {
        ...
        try {
          ...
          if (ownerSnap.exists()) {
            ...
            const bizDb = data.db;
            let subUsers: User[] = [];
            try { const us = await getDocs(collection(db, 'businesses', uid, 'users')); subUsers = us.docs.map((d) => d.data() as User); } catch {}
            const ownerUser = subUsers.find((u: User) => u.role === 'owner') || bizDb?.users?.find((u: User) => u.role === 'owner') || {
              id: uid,
  ...
  const loginWithGoogle = useCallback(async () => {
    ...
    try {
      ...
      if (!existing.exists() && !isExistingTeamMember) {
        ...
        if (REGION !== 'US') {
          newCfg = { ...newCfg, lang: REGION_DEFAULTS.lang, currency: REGION_DEFAULTS.currency, region: REGION, tax_rate: REGION_DEFAULTS.taxRate, tax_label: REGION_DEFAULTS.taxLabel, timezone: REGION_DEFAULTS.timezone };
        }
