          ...
          {techPayrolls.map((tp) => (
            <Card key={String(tp.tech.id)}>
              <Box sx={{ p: '16px', display: 'flex', alignItems: 'center', gap: '16px', '@media(max-width:768px)': { flexDirection: 'column', alignItems: 'stretch' } }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 200 }}>
                  <Box sx={{
                    ...
                    bgcolor: tp.commissionAmount > 0 ? 'rgba(0,229,176,0.15)' : 'rgba(255,255,255,0.05)', fontSize: 18,
                  }}>
                    👷
                  </Box>
                  <Box>
                    <Typography sx={{ fontSize: 14, fontWeight: 700 }}>{tp.tech.name}</Typography>
                    <Typography sx={{ fontSize: 11, color: '#5a7080' }}>
                ...
                <Box sx={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', '@media(max-width:768px)': { gridTemplateColumns: '1fr 1fr' } }}>
                  <Box sx={{ textAlign: 'center', p: '8px', bgcolor: 'rgba(255,255,255,0.025)', borderRadius: '8px' }}>
                    ...
                    <Typography sx={{ fontSize: 9, color: '#5a7080', fontWeight: 600 }}>JOBS</Typography>
                  </Box>
                  <Box sx={{ textAlign: 'center', p: '8px', bgcolor: 'rgba(255,255,255,0.025)', borderRadius: '8px' }}>
                    <Typography sx={{ fontSize: 16, fontWeight: 800 }}>{formatCurrency(tp.totalRevenue, currency)}</Typography>
                    <Typography sx={{ fontSize: 9, color: '#5a7080', fontWeight: 600 }}>REVENUE</Typography>
                  </Box>
                  <Box sx={{ textAlign: 'center', p: '8px', bgcolor: 'rgba(255,255,255,0.025)', borderRadius: '8px' }}>
                    <Typography sx={{ fontSize: 16, fontWeight: 800, color: '#ff4d6d' }}>{formatCurrency(tp.totalMaterials, currency)}</Typography>
                    <Typography sx={{ fontSize: 9, color: '#5a7080', fontWeight: 600 }}>MATERIALS</Typography>
                  </Box>
                  <Box sx={{ textAlign: 'center', p: '8px', bgcolor: 'rgba(0,229,176,0.08)', borderRadius: '8px', border: '1px solid rgba(0,229,176,0.2)' }}>
                    <Typography sx={{ fontSize: 16, fontWeight: 800, color: '#00e5b0' }}>{formatCurrency(tp.commissionAmount, currency)}</Typography>
                    <Typography sx={{ fontSize: 9, color: '#5a7080', fontWeight: 600 }}>COMMISSION</Typography>
                  </Box>
                </Box>
                ...
                <Button size="small" onClick={() => openPayslip(tp)} sx={{
                  ...
                  bgcolor: 'rgba(0,229,176,0.08)', color: '#00e5b0',
              ...
              {tp.completedJobs.length > 0 && (
                <Box sx={{ borderTop: '1px solid rgba(255,255,255,0.055)' }}>
                  <DataTable<Job>
                    keyExtractor={(j) => j.id}
                    columns={[
                      ...
                      { key: 'commission', label: 'עמלה', render: (j) => (
                        <Typography sx={{ fontWeight: 700, color: '#00e5b0', fontSize: 12 }}>
      ...
      <ModalBase open={showPayslip} onClose={() => setShowPayslip(false)} title={`Payslip — ${selectedTech?.tech.name || ''}`} maxWidth={500}
        footer={
          <Button size="small" onClick={() => { toast('📋 Payslip copied to clipboard!'); setShowPayslip(false); }}
            sx={{ bgcolor: 'rgba(0,229,176,0.08)', color: '#00e5b0', border: '1px solid rgba(0,229,176,0.2)', borderRadius: '10px', fontWeight: 700, '&:hover': { bgcolor: '#00e5b0', color: '#000' } }}>
            📋 Copy Payslip
          </Button>
        }>
        {selectedTech && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {/* Header */}
            <Box sx={{ bgcolor: 'rgba(0,229,176,0.08)', border: '1px solid rgba(0,229,176,0.2)', borderRadius: '10px', p: '14px 16px' }}>
              <Typography sx={{ fontFamily: "'Syne', sans-serif", fontSize: 14, fontWeight: 800, mb: '6px' }}>
                {cfg.biz_name || 'Business'} — Payslip
              </Typography>
              <Typography sx={{ fontSize: 11, color: '#a8bcc8', lineHeight: 1.8 }}>
            ...
            {/* Breakdown */}
            <Box sx={{ bgcolor: '#141920', border: '1px solid rgba(255,255,255,0.055)', borderRadius: '10px', p: '14px 16px' }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: '8px' }}>
                <Typography sx={{ fontSize: 12, color: '#a8bcc8' }}>{L("Jobs Completed","עבודות שהושלמו")}</Typography>
                <Typography sx={{ fontSize: 12, fontWeight: 700 }}>{selectedTech.jobCount}</Typography>
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: '8px' }}>
                <Typography sx={{ fontSize: 12, color: '#a8bcc8' }}>{L("Total Revenue Generated","סה\"כ הכנסה")}</Typography>
                <Typography sx={{ fontSize: 12, fontWeight: 700, color: '#22c55e' }}>{formatCurrency(selectedTech.totalRevenue, currency)}</Typography>
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: '8px' }}>
                <Typography sx={{ fontSize: 12, color: '#a8bcc8' }}>{L("Materials Cost","עלות חומרים")}</Typography>
                <Typography sx={{ fontSize: 12, color: '#ff4d6d' }}>-{formatCurrency(selectedTech.totalMaterials, currency)}</Typography>
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: '8px' }}>
                <Typography sx={{ fontSize: 12, color: '#a8bcc8' }}>{L("Commission Rate","אחוז עמלה")}</Typography>
                <Typography sx={{ fontSize: 12, fontWeight: 600 }}>{selectedTech.commissionRate}%</Typography>
              </Box>
              <Box sx={{ height: '1px', bgcolor: 'rgba(255,255,255,0.055)', my: '8px' }} />
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography sx={{ fontSize: 16, fontWeight: 800 }}>{L("Total Pay","סה\"כ תשלום")}</Typography>
                <Typography sx={{ fontSize: 16, fontWeight: 800, color: '#00e5b0' }}>{formatCurrency(selectedTech.commissionAmount, currency)}</Typography>
              </Box>
            </Box>
            ...
            {selectedTech.completedJobs.length > 0 && (
              <Box>
                <Typography sx={{ fontSize: 10, fontWeight: 700, color: '#5a7080', textTransform: 'uppercase', letterSpacing: '0.5px', mb: '8px' }}>
                  Job Details
                </Typography>
                {selectedTech.completedJobs.map((j) => (
                  <Box key={j.id} sx={{ display: 'flex', justifyContent: 'space-between', py: '6px', borderBottom: '1px solid rgba(255,255,255,0.035)' }}>
                    ...
                    <Box sx={{ textAlign: 'right' }}>
                      <Typography sx={{ fontSize: 11, color: '#a8bcc8' }}>{formatCurrency(j.revenue || 0, currency)}</Typography>
                      <Typography sx={{ fontSize: 11, fontWeight: 700, color: '#00e5b0' }}>
