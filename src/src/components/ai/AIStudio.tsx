...
type Tab = 'marketing' | 'post' | 'landing' | 'gallery';
...
export function AIStudio() {
  ...
  return (
    <Box sx={{ maxWidth: 800, mx: 'auto' }}>
      ...
      {tab === 'gallery' && <GalleryTab bizId={bizId} />}
    </Box>
  );
}
...
// ===== Photo → Post =====
function PostTab({ bizId }: { bizId: string | null }) {
...
// ===== Gallery (UI; storage upload requires Firebase Storage config) =====
function GalleryTab({ bizId }: { bizId: string | null }) {
  ...
  return (
    <Box>
      <Typography sx={{ fontSize: 22, fontWeight: 800, color: c.text, mb: 1 }}>גלריית עבודות</Typography>
      <Typography sx={{ fontSize: 14, color: c.text2, mb: 3 }}>
        העלה תמונות של העבודות שלך. הן יופיעו בדף הנחיתה האוטומטי שלך.
      </Typography>

      <Button component="label" variant="outlined" sx={{ py: 1.5, px: 3, borderRadius: 3, mb: 3, borderStyle: 'dashed' }}>
        📷 העלה תמונות
      ...
      <Box sx={{ bgcolor: c.surface1, border: `1px solid ${c.border}`, borderRadius: 2, p: 2, mt: 3 }}>
        <Typography sx={{ fontSize: 12, color: c.text3 }}>
          💡 כדי לשמור תמונות לצמיתות צריך להפעיל Firebase Storage. כרגע התמונות מוצגות לתצוגה מקדימה.
        </Typography>
      </Box>
    </Box>
  );
