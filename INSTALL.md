...
**תפקידים:** Owner (הכל) · Partner (הכל חוץ מצוות/הגדרות) · Dispatcher (לקוחות/הצעות/קבלות/עבודות/סגירות, בלי סכומי כסף בדשבורד) · Technician (**רק** העבודות שלו והסגירות שלו).

**הזמנה:** Team → Invite member → שם, אימייל, טלפון, תפקיד → נוצר לינק אישי `/join/<token>` (+ הודעת WhatsApp מוכנה). החבר נרשם **עם האימייל שהזנת** (סיסמה או Google) ונכנס ישר לעסק. אימייל אחר = חסום. "Remove access" מנתק מיד.

**עבודות (Jobs):** יצירה מהצעה מאושרת או ידנית, תאריך/שעה/משך, שיבוץ טכנאי, סטטוסים Scheduled → On the way → In progress → Completed, ניווט Google Maps + חיוג. **סגירה של טכנאי = מתוך עבודה בלבד** ("Close job" → סכום, מקדמה ולאן, יתרה ולאן, חומרים, הערות, תמונות) — נרשמת על שמו. Owner רואה בדשבורד "This week by technician" ובסגירות פילטר לפי טכנאי.

**אכיפה אמיתית ב-Rules (לא רק בתפריט):** טכנאי יכול לקרוא/לכתוב רק `jobs`/`closings` עם `techUid == uid` שלו; `members/{uid}.role` חייב להיות שווה למה שה-Owner קבע ב-`tech_lookup` (אין קידום עצמי). שכבת הנתונים של טכנאי מאזינה רק לשני האוספים האלה.

**חובה לפרוס rules מחדש** (`firestore.rules.shared` / `firestore.rules`) — אחרת טכנאים לא יראו כלום.

---

## V1.2 — אחוזים, מחירון, דוחות, לידים, תמונות, ביקורות, מפה חיה

**אחוזים (החלוקה עם חברות אחרות)**
- בכל **עבודה** ובכל **סגירה**: `Job came from` (My own או שם חברה) · `We keep %` · מתג `Take materials off the top`.
- ברירת מחדל ב-Settings → **Commission & job sources** (אחוז + רשימת חברות). האחוז נשמר **על העבודה**, כך שהיסטוריה לא משתנה אם תשנה את ברירת המחדל.
- חישוב חי בסגירה: סכום → פחות חומרים → כמה החברה לוקחת → **כמה נשאר לנו**.
- הסגירות נשמרות עם `ourShare`/`companyShare` מוקפאים. סגירות ישנות בלי אחוז = 100% שלנו (שום דבר לא משתנה רטרואקטיבית).
- דשבורד: "Our share this week". סגירות: סיכום שבועי + "Where the work came from" + פילטר לפי חברה. CSV עם כל העמודות.
- **טכנאי לא רואה אחוזים בכלל** — לא בעבודה, לא בסגירה, לא בדוחות.

**Price book** (`/products`) — שירותים וחלקים עם מחיר ועלות, מרווח, כמה פעמים הוצע. ההצעות והקבלות מתמלאות מכאן. יש רשימת התחלה מוכנה לארובות/דלתות מוסך.

**Reports** (`/reports`, owner/partner) — חודש/רבעון/שנה/טווח חופשי: ברוטו, החלק שלנו, חומרים, ממוצע לעבודה, לא נגבה, גרף 6 חודשים, פילוח לפי חברה־מקור / סוג עבודה / טכנאי, אחוז המרה של הצעות, CSV.

**Leads** (`/leads`) — שיחה נכנסת → ליד (שם + טלפון מספיק), סטטוסים, תזכורת follow-up, חיוג/וואטסאפ בקליק, והמרה ל**הצעה** או ל**עבודה** (יוצר לקוח אוטומטית).

**Photos** (`/photos`) — כל התמונות מסודרות לפי עבודה, חיפוש, הוספה בדיעבד, תצוגה מלאה והורדה.

**Reviews** (`/reviews`) — אחרי סגירה: "Ask" → הודעה מוכנה עם לינק הביקורת ב-Google (Settings → Reviews) ב-SMS / וואטסאפ / מייל, והיסטוריית בקשות.

**Live map** (`/gps-tracking`) — מיקום הטכנאים בזמן אמת על מפה (OpenStreetMap, בלי מפתח ובלי חיוב), צבע לכל אחד, מתי עודכן לאחרונה, העבודות שלו היום, וניווט בקליק.

---

## V1.3 — שיוך לבעל הבית/שותף, אחוז לכל חברה, שליחת לו״ז ב-SMS

**אחוז לכל חברה** — Settings → Commission & job sources: טבלה עם שם חברה · כמה אנחנו שומרים · האם חומרים יורדים לפני החלוקה. בחירת החברה בעבודה/סגירה מושכת את האחוז שלה אוטומטית (וניתן לשנות ידנית לעבודה בודדת). למטה "Default we keep" — רק לחברה שעוד לא ברשימה.
> באג שתוקן: השדה הישן היה Autocomplete וחייב Enter אחרי כל שם — אם הקלדת ולחצת Save, זה נשמר ריק. עכשיו זו טבלה עם שורות אמיתיות. שמות שכבר הזנת עוברים אוטומטית עם אחוז ברירת המחדל.

**Assigned to** — עבודה משויכת עכשיו ל**בעל הבית**, ל**שותף** או לטכנאי (אדם אחד לעבודה). לכל אחד צבע ביומן, פילטר משלו, ו"Only mine" לבעל הבית. עבודה של בעל הבית/שותף מתויגת כ-`owner`/`partner` בסגירה — הכסף שייך לעסק, והחלוקה ביניכם בסוף החודש היא מחוץ למערכת. טכנאי = **תיוג בלבד**, בלי חישוב עמלה.

**שותף = תפקיד שטח** — רואה רק את העבודות והסגירות שלו, בדיוק כמו טכנאי, **גם ברמת ה-Rules** (לא רק בתפריט). אם תרצה שותף שרואה הכל — זה תפקיד `dispatcher` או חשבון owner.

**שליחת לו״ז ב-SMS** — בלוח השנה או בדשבורד: **Send tomorrow** → רשימת כל מי שיש לו עבודות מחר, הודעה מוכנה לכל אחד (שעה, לקוח, כתובת, טלפון, הערות), ניתנת לעריכה, ונשלחת ב-**SMS / וואטסאפ / מייל**, או "Send all by SMS" לכולם. עבודות ללא שיוך מסומנות באזהרה. וואטסאפ עובד בלי שום הגדרה; SMS דורש מספר Twilio.

**חשבון שדה אישי** — Team → "Create my field login": יוצר הזמנה לכתובת `you+field@gmail.com` (מגיעה לאותה תיבה). נרשם איתה → מקבל תצוגה של טכנאי שרואה רק את העבודות שלו. אלטרנטיבה בלי חשבון שני: הצ'יפ **Only mine** בעמוד Jobs.

---

## V1.4 — יומן חופף, דיספאצ'ר מבודד

**יומן** — שתי עבודות באותה שעה כבר לא יושבות אחת על השנייה: הן מתחלקות לעמודות זו לצד זו (שתיים = חצי רוחב, שלוש = שליש). עבודה בלי חפיפה נשארת ברוחב מלא, ובזמן גרירה היא חוזרת לרוחב מלא כדי להישאר קריאה.

**Dispatcher = רק העבודות של החברה שלנו** — רואה לידים, לקוחות, יומן ועבודות — אבל **רק עבודות ש-100% שלנו**. עבודה שנמשכה מ-Tal / Barak-ziv / Vadim פשוט לא קיימת עבורו: לא ביומן, לא ברשימה, לא בחיפוש. בנוסף הוא לא רואה שום סכום כסף (לא הצעות, לא קבלות, לא סגירות, לא אחוזים) ולא יכול לסגור עבודה.

האכיפה בשלוש שכבות:
1. **שכבת הנתונים** — הוא נרשם רק ל-`leads`, `customers` ו-`jobs where ownWork == true`.
2. **Rules** — גם אם יפתח קונסול, קריאה לעבודה עם `ownWork=false` או לכל אוסף אחר (quotes / receipts / closings / products / reports) נחסמת בשרת.
3. **כתיבה** — כל עבודה שהוא יוצר נשמרת בכוח כ-`ownWork: true, source: ''`, כך שהוא לא יכול "להבריח" עבודה משוכה פנימה.

לכל עבודה נוסף שדה `ownWork`, ועבודות קיימות מתעדכנות אוטומטית בכניסה הראשונה שלך כ-owner (לפי המקור שלהן).

**טבלת הרשאות**

| | Owner | Partner | Dispatcher | Technician |
|---|---|---|---|---|
| לידים | ✔ | — | ✔ | — |
| לקוחות | ✔ | — | ✔ (בלי כסף) | — |
| עבודות | הכל | שלו בלבד | **רק 100% שלנו** | שלו בלבד |
| יומן | הכל | שלו | רק 100% שלנו | שלו |
| סגירות | ✔ | שלו | — | שלו |
| הצעות / קבלות | ✔ | — | — | — |
| דוחות / אחוזים / מחירון | ✔ | — | — | — |
| צוות / הגדרות | ✔ | — | — | — |

> **חובה לפרוס Rules מחדש** אחרי העדכון הזה — אחרת הדיספאצ'ר עדיין יוכל לקרוא הכל.
